...
<body>
   ...
   <nav class="navbar navbar-expand-lg navbar-dark" style="background-color: #a42744;color: white">
    ...
    <div class="">
      <br>
    <a class="" href="createUserClub.php">Create User-Club</a>
<?php
require('db.php');   

$query = "SELECT uc.id, Concat(us.first_name, ' ', us.last_name) as user_name, us.email, cl.name as club_name, ca.name as category_name, uc.active
          FROM user_club uc 
          LEFT JOIN users us on us.id = uc.user_id 
          LEFT JOIN clubs cl on cl.club_id = uc.club_id 
          LEFT JOIN category ca on ca.category_id = cl.category_id 
          ORDER BY us.id;";
...
echo '<table border="0" cellspacing="2" cellpadding="2"> 
      <tr> 
          ...
          <td> <font face="Arial">Is Active</font> </td>        
...
if ($result = $conn->query($query)) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $field4name = $row["club_name"]; 
        $field5name = $row["category_name"]; 
        $field6name = $row["active"];
        ...
        echo '<tr> 
                  ...
                  <td>'.$field5name.'</td> 
                  <td>'.$field6name.'</td> 
                  <td align="center"><a href="editUserClub.php?id='.$field1name.'">Edit</a></td>
                  <td align="center"><a onclick="return checkDelete()" href="deleteUserClub.php?id='.$field1name.'">Delete</a></td>
              </tr>';
    }
    $result->free();
} 
