...
<body>
   ...
   <nav class="navbar navbar-expand-lg navbar-dark" style="background-color: #a42744;color: white">
    ...
    <div class="">
      <br>
    <a class="" href="createClub.php">Create Club</a>
<?php
require('db.php');   

$query = "SELECT cl.club_id, cl.name as club_name, ca.name category_name, cl.benefits, cl.days, 
cl.fromTime, cl.toTime, cl.fromAge, cl.toAge, cl.location, cl.price  FROM clubs cl 
LEFT JOIN category ca on cl.category_id = ca.category_id ORDER BY cl.club_id;";

echo '<table border="0" cellspacing="2" cellpadding="2"> 
      <tr> 
          <td> <font face="Arial">SL</font> </td>          
          <td> <font face="Arial">Club Name</font> </td> 
          <td> <font face="Arial">Category Name</font> </td> 
          <td> <font face="Arial">Benefits</font> </td> 
          <td> <font face="Arial">Days</font> </td>           
          <td> <font face="Arial">From Age</font> </td>
          <td> <font face="Arial">To Age</font> </td>
          <td> <font face="Arial">From Time</font> </td>
          <td> <font face="Arial">To Time</font> </td>
...
if ($result = $conn->query($query)) {
    ...
    while ($row = $result->fetch_assoc()) {
        $field1name = $row["club_id"];
        $field2name = $row["club_name"];
        $field3name = $row["category_name"];
        $field4name = $row["benefits"];
        $field5name = $row["days"];
        $field6name = $row["fromAge"]; 
        $field7name = $row["toAge"]; 
        $field8name = $row["fromTime"]; 
        $field9name = $row["toTime"]; 
        $field10name = $row["location"]; 
        $field11name = $row["price"]; 
        ...
        echo '<tr> 
                  ...
                  <td>'.$field6name.'</td>  
                  <td>'.$field7name.'</td>  
                  <td>'.date("h:i:sa", strtotime($field8name)).'</td>  
                  <td>'.date("h:i:sa", strtotime($field9name)).'</td>  
                  <td>'.$field10name.'</td>  
                  <td>'.$field11name.'</td>  
                  <td align="center"><a href="editClub.php?id='.$field1name.'">Edit</a></td>
                  <td align="center"><a onclick="return checkDelete()" href="deleteClub.php?id='.$field1name.'">Delete</a></td>
              </tr>';
    }
    $result->free();
} 
