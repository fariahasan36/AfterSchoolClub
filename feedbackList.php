...
$query = "SELECT fd.id, concat(us.first_name, ' ', us.last_name) as user_name, us.email, fd.feedback FROM feedback fd
          left join users us on us.id = fd.user_id ORDER BY fd.user_id ASC;";
...
if ($result = $conn->query($query)) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<tr> 
                  ...
                  <td>'.$field3name.'</td> 
                  <td>'.$field4name.'</td> 
                  <td align="center"><a onclick="return checkDelete()" href="deleteFeedback.php?id='.$field1name.'">Delete</a></td>
              </tr>';
    }
    $result->free();
} 
